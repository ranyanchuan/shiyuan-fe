 ...
 * @param {*} successMsg 成功提示
 */
export const processDataPlatForm = (response,successMsg) => {
    if(typeof response != 'object') {
        Error('数据返回出错：1、请确保服务运行正常；2、请确保您的前端工程代理服务正常；3、请确认您已在本地登录过应用平台');
        return;
    }
    if(response.status=='401'){
        Error(`错误:${(response.msg)}`);
        return;
    }
    if(response.status=='200'){

        let data=response;
        let repMsg = data.data.status;
        if(repMsg =='success'){
            if(successMsg){
                success(successMsg);
            }
            return data;
        }else if(repMsg=='failed'){
        Error(`错误:${(data && data.data.msg && convert(data.data.msg)) || '数据返回出错'}`);
        }else {
        Error(`错误:${convert(data.data.msg)}`);
            return;
        }
    }else{
        Error('请求错误');
