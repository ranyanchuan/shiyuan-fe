...
class IndexView extends Component {
    constructor(props) {
        ...
        reflicenseIdDropData: null,
        refuserDropData: null,
    filterable: false,
        }
    ...
    onRefreshList = () => {
        actions.popupEditlicenseUser.loadList(this.props.queryParam);
    }
    ...
    confirmGoBack(type) {
        ...
        if (type === 1) { // 确定
            ...
            actions.popupEditlicenseUser.removeList(record);
        }
    }
...
afterRowFilter = (status) => {
    if (!status) { 
        ...
        actions.popupEditlicenseUser.updateState({queryParam}); //缓存查询条件
        actions.popupEditlicenseUser.loadList(queryParam);
    }
